...
class App extends Component {
  ...
  addTask(e) {
    if (this.state._id) {
      fetch(`/api/tasks/${this.state._id}`, { // Corrección: Mover la llave de cierre '}' antes del paréntesis final
        method: 'PUT',
                    ...
                    this.state.tasks.map(task=> {
                      return (
                        <tr key={task._id} > 
                          ...
                          <td>
                            <button className='btn light-blue darjet-4' style={{margin: '4px'}} onClick={ ()=> this.deleteTaks(task._id)}>
                              <i className='material-icons' >delete</i>
                            </button>
                            <button className='btn light-blue darjet-4' >
